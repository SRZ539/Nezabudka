...
public sealed class LocalizationService : INotifyPropertyChanged
{
    ...
    private static IReadOnlyDictionary<AppLanguage, IReadOnlyDictionary<string, string>> CreateCatalogs()
    {
        ...
        [
            ...
            ("DataManagement", "Данные и настройки", "Data and settings", "数据与设置"),
            ("DataFolder", "Папка заметок", "Notes folder", "笔记文件夹"),
            ("ExportNotes", "Экспорт", "Export", "导出"),
            ("ImportNotes", "Импорт", "Import", "导入"),
            ("ChangeDataFolder", "Изменить папку данных", "Change data folder", "更改数据文件夹"),
            ("ChooseDataFolder", "Выберите папку для заметок Незабудки", "Choose a folder for Forget-Me-Not notes", "选择勿忘我笔记文件夹"),
            ("ArchiveFileFilter", "Архив Незабудки (*.nezabudka)|*.nezabudka|Все файлы (*.*)|*.*", "Forget-Me-Not archive (*.nezabudka)|*.nezabudka|All files (*.*)|*.*", "勿忘我存档 (*.nezabudka)|*.nezabudka|所有文件 (*.*)|*.*"),
            ("ImportModePrompt", "Нажмите «Да», чтобы объединить заметки. Нажмите «Нет», чтобы заменить все текущие заметки. Перед импортом будет создана резервная копия.", "Choose Yes to merge notes, or No to replace all current notes. A backup will be created first.", "选择“是”合并笔记，选择“否”替换所有当前笔记。导入前会创建备份。"),
            ("ExportCompleted", "Все заметки экспортированы.", "All notes were exported.", "所有笔记已导出。"),
            ("ImportCompleted", "Импортировано заметок: {0}.", "Imported notes: {0}.", "已导入笔记：{0}。"),
            ("DataFolderChanged", "Папка изменена. Если в ней уже были заметки, они безопасно объединены с текущими.", "The folder was changed. Existing notes there were safely merged with the current notes.", "文件夹已更改。若其中已有笔记，已与当前笔记安全合并。"),
            ("OperationFailed", "Операция не выполнена. Проверьте файл или доступ к выбранной папке.", "The operation failed. Check the file and access to the selected folder.", "操作失败。请检查文件以及所选文件夹的访问权限。"),
            ("AutosaveSetting", "Автосохранение", "Autosave", "自动保存"),
            ("GlobalHotkeysSetting", "Глобальные сочетания клавиш", "Global keyboard shortcuts", "全局快捷键"),
            ("GlobalHotkeysHint", "Ctrl+Shift+Space — показать приложение · Ctrl+Shift+N — новая заметка", "Ctrl+Shift+Space — show the app · Ctrl+Shift+N — new note", "Ctrl+Shift+Space — 显示应用 · Ctrl+Shift+N — 新建笔记"),
            ("GlobalHotkeysUnavailable", "Не удалось зарегистрировать сочетания: одно из них уже используется другой программой.", "The shortcuts could not be registered because another app is already using one of them.", "无法注册快捷键，其中一个已被其他应用使用。"),
            ...
            ("Pinned", "Закреплено", "Pinned", "已置顶"),
            ("PinNote", "Закрепить", "Pin", "置顶"),
            ("UnpinNote", "Открепить", "Unpin", "取消置顶"),
            ("TrashWithCount", "Корзина · {0}", "Trash · {0}", "回收站 · {0}"),
            ("TrashEmpty", "Корзина пуста", "Trash is empty", "回收站为空"),
            ("RestoreNote", "Восстановить", "Restore", "恢复"),
            ("EmptyTrash", "Очистить корзину", "Empty trash", "清空回收站"),
            ("EmptyTrashTitle", "Очистка корзины", "Empty trash", "清空回收站"),
            ("EmptyTrashPrompt", "Безвозвратно удалить все заметки из корзины?", "Permanently delete every note in the trash?", "永久删除回收站中的所有笔记吗？"),
            ...
            ("AutosaveDisabled", "Автосохранение выключено · Ctrl+S для сохранения", "Autosave is off · press Ctrl+S to save", "自动保存已关闭 · 按 Ctrl+S 保存"),
            ("FooterCredit", "Разработано на C# · KlaOs · альфа 0.2", "Built with C# · KlaOs · alpha 0.2", "使用 C# 开发 · KlaOs · Alpha 0.2"),
            ...
            ("NoteMovedToTrash", "Заметка перемещена в корзину", "Note moved to trash", "笔记已移至回收站"),
            ("UndoDelete", "Вернуть", "Restore", "恢复"),
            ("SaveBeforeExitPrompt", "Есть несохранённые изменения. Сохранить их перед выходом?", "There are unsaved changes. Save them before quitting?", "有未保存的更改。退出前保存吗？"),
            ("UnsavedChangesTitle", "Несохранённые изменения", "Unsaved changes", "未保存的更改"),
            ...
            ("SaveUnsaved", "Есть несохранённые изменения", "There are unsaved changes", "有未保存的更改"),
